...
def _create_localization_list(params, user):
    """Localization POST method in its own function for reuse by Migrate endpoint."""
    # Check that we are getting a localization list.
    try:
        loc_specs = params["body"]
    except KeyError:
        raise ValueError("Localization creation requires list of localizations!")
    ...
    for loc in loc_specs:
        meta_ids.add(loc["type"])
        media_ids.add(loc["media_id"])
        version_ids.add(loc.get("version", None))
    ...
    attr_specs = [
        check_required_fields(required_fields[loc["type"]][0], required_fields[loc["type"]][2], loc)
        for loc in loc_specs
    ]
    ...
    ids = bulk_log_creation(localizations, project, user)
...
class LocalizationListAPI(BaseListView):
    ...
    def _post(self, params):
        return _create_localization_list(params, self.request.user)
