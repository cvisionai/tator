...
export class DownloadDialog extends ModalDialog {
  constructor() {
    super();

    this._title.nodeValue = "Downloading Files";
    ...
    this._downloadText = document.createElement("h3");
    this._downloadText.setAttribute("class", "text-center text-semibold py-3");
    this._main.appendChild(this._downloadText);
  ...
  init(store) {
    ...
    store.subscribe(
      (state) => state.downloadTotalFiles,
      this._setTotalFiles.bind(this)
    );
    store.subscribe(
      (state) => state.downloadFilesComplete,
      this._setFilesCompleted.bind(this)
    );
    store.subscribe(
      (state) => state.downloadCurrentFile,
      this._setFilename.bind(this)
    );
    store.subscribe((state) => state.downloadError, this._addError.bind(this));
  }
  ...
  _setTotalFiles(numFiles) {
    ...
    this._fileText.textContent = `Downloaded 0/${this._totalFiles} Files`;
  }
  ...
  _setFilename(currentFile) {
    if (currentFile === null) {
      this._downloadText.style.display = "none";
    } else {
      this._downloadText.textContent = `Downloading ${currentFile}...`;
    }
  }
  ...
  _fileComplete() {
    ...
    this._fileText.textContent = `Downloaded ${this._doneFiles}/${this._totalFiles} Files`;
  ...
  _finish() {
    if (!this._cancelled) {
      ...
      if (this._failFiles == 0) {
        ...
        this._title.nodeValue = "Download Failure!";
      }
    }
  }
