...
export class BrowserRecommendation extends TatorElement {
   ...
   init(root) {
      ...
      const sessionValue = sessionStorage.getItem(`handle_error__browser-support`);
      if (browserName !== "chrome" && browserName !== "edge" && sessionValue == null) {
         ...
         modalError.init("Warning Unsupported Browser", msg_html, 'error', 'Exit', true);
         modalError.setAttribute("is-open", "");
         sessionStorage.setItem(`handle_error__browser-support`, 'true');
         sessionStorage.setItem(`handle_error__browser-support_${document.location.pathname}`, 'true');
      }
   }
