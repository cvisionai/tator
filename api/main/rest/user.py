...
def handle_avatar_management(user, params):
    ...
    if new_avatar:
        ...
        if len(img_data) > MAX_PROFILE_IMAGE_SIZE:
            raise ValueError(
                f"Supplied profile image is too large {len(img_data)} > {MAX_PROFILE_IMAGE_SIZE}"
            )
...
def user_serializer_helper(response_data, presigned_ttl):
    ...
    if type(response_data) == ReturnList:
        ...
        response_data["profile"]["avatar"] = generic_store.get_download_url(
            avatar_key, presigned_ttl
        )
    return response_data
...
class UserListAPI(BaseListView):
    ...
    def _get(self, params):
        ...
        if elemental_id is not None:
            users = User.objects.filter(elemental_id=elemental_id)
        return user_serializer_helper(
            UserSerializerBasic(users, many=True).data, params.get("presigned", None)
        )
    ...
    def _post(self, params):
        ...
        if registration_token is None:
            ...
            invites = Invitation.objects.filter(
                registration_token=registration_token, status="Pending"
            )
            if invites.count() == 0:
                ...
                Affiliation.objects.create(
                    organization=invite.organization, permission=invite.permission, user=user
                )

        handle_avatar_management(user, params)

        return {"message": f"User {username} created!", "id": user.id}
...
class UserDetailAPI(BaseDetailView):
    ...
    def _patch(self, params):
        ...
        if "password" in params:
            ...
            resets = PasswordReset.objects.filter(reset_token=params["reset_token"], user=user)
            if resets.count() == 0:
